...
//  GifSearch
//
//  Created by Luda Parfenova on 14/08/2021.
//
...
class Helpers {
    static func getGIFURLs(result: JSON) -> [URL] {
        Root(gifData: result).data.map{$0.images.downsized.url}
    }
}
    
    extension UIImageView {
        func setGIFImage(url: URL, repeatCount: Int = 0 ) {
            DispatchQueue.global().async {
                if let gif = UIImage.makeGIFFromURLData(url: url, repeatCount: repeatCount) {
    ...
    extension UIImage {
        class func makeGIFFromURLData(url: URL, repeatCount: Int = 0) -> GIF? {
            let data = try? Data(contentsOf:url )
            guard let d = data else {
                print("Cannot turn image named \"\(url)\" into data")
                return nil
            }
